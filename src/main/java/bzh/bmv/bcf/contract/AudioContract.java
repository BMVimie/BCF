package bzh.bmv.bcf.contract;

/**
 * Audio class contract.
 * This defines constants relative to the audio class.
 * @author REVEREAU
 * @since 2018-09-13
 */
public class AudioContract {
	/**
	 * Constant to reference Audio table name.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String TABLENAME = "bcf_audio";
	/**
	 * Constant to reference Audio.audioId.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String AUDIOID = "audioId";
	/**
	 * Constant to reference Audio.audioName.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String AUDIONAME = "audioName";
	/**
	 * Constant to reference Audio.audioSrc.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String AUDIOSRC = "audioSrc";
	/**
	 * Constant to reference Audio.audioExtension.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String AUDIOEXTENSION = "audioExtension";
	/**
	 * Constant to reference Audio.audioDuration.
	 * @see bzh.bmv.bcf.model.Audio
	 */
	public static final String AUDIODURATION = "audioDuration";
}
